...
class Storage:
    ...
    def save_alert(self, game: GameSnapshot, decision: Decision) -> None:
        self.conn.execute(
            """
    ...
    def performance(self) -> dict[str, Any]:
        rows = self.conn.execute("select status, count(*) as total from alerts group by status").fetchall()
        ...
        return {"summary": summary, "settled": settled, "win_rate": win_rate, "profit_units": round(float(profit), 2)}
